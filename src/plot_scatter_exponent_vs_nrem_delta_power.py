...
def analyze_multiple_subjects(eeg_pickle_paths, sleep_stage_csv_paths, fg_models_pickle_paths, exclusion_ranges=None):
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(df['power'], df['exponent'])
    
    # Plot
    plt.figure(figsize=(10, 8))
    sns.scatterplot(data=df, x='power', y='exponent', alpha=0.6, s=80)
    sns.regplot(data=df, x='power', y='exponent', scatter=False, 
                color='red', line_kws={'linestyle': '-'}, ci=None)
    ...
    plt.tight_layout()
    plt.savefig('/ceph/harris/volkan/fooof/plots/delta_power/nrem_delta_vs_exponent_all_subjects.png', dpi=600, bbox_inches='tight')
    plt.show()
